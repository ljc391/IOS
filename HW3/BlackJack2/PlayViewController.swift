...
class PlayViewController: UIViewController, UIActionSheetDelegate {
    ...
    func dealcards() {
        ...
        if numofplayers == 1 && aidied == 0{
            for i in 0..<2 {
                ...
                naiplayer.number += [cardnumber]
                naiplayer.suit += [cardsuit]
            }
            
            naiplayer.numberofcards = 2
            naiplayer.sum = countSum(naiplayer.number, size: naiplayer.numberofcards)
    ...
    func countaiplayer(){
        println("aiplayer's time")
        //
        var maxsum:Int = players[0].sum
        naiplayer.bet = 10
        naiplayer.money = naiplayer.money - naiplayer.bet
        Imoney.text = String(naiplayer.money)
        Ibet.text = String(naiplayer.bet)
        naiplayer.sum = countSum(naiplayer.number, size: naiplayer.numberofcards)
        if HorS == 1 {//S
            while aigcard == 0  && HorS == 1{
                if adealer.number[0] == 2 || adealer.number[0] == 3{
                    if naiplayer.sum <= 12 {
                        println("AI give card")
                        randomcard = Int(rand()%numofcards)
                        randomcardnumber = randomcard%52
                        while used[randomcard] == 0{
                            randomcard = Int(rand()%numofcards)
                        ...
                        naiplayer.number += [cardnumber]
                        naiplayer.suit += [cardsuit]
                        naiplayer.numberofcards++
                        naiplayer.sum = countSum(naiplayer.number, size: naiplayer.numberofcards)
                        
                        
                        
                    } else if adealer.number[0] == 4 || adealer.number[0] == 5 || adealer.number[0] == 6{
                        if naiplayer.sum <= 11 {
                            println("AI give card")
                            randomcard = Int(rand()%numofcards)
                            randomcardnumber = randomcard%52
                    ...
                    } else if adealer.number[0] == 7 || adealer.number[0] == 8 || adealer.number[0] == 9 || adealer.number[0] == 10 || adealer.number[0] == 1{
                        if naiplayer.sum <= 16 {
                            println("AI give card")
                            randomcard = Int(rand()%numofcards)
                            randomcardnumber = randomcard%52
            ...
            while aigcard == 0  && HorS == 1{
                if adealer.number[0] == 1 || adealer.number[0] == 2 || adealer.number[0] == 7 || adealer.number[0] == 8{
                    if naiplayer.sum <= 12 {
                        println("AI give card")
                        randomcard = Int(rand()%numofcards)
                        randomcardnumber = randomcard%52
                ...
                } else if adealer.number[0] == 3 || adealer.number[0] == 4 || adealer.number[0] == 5 || adealer.number[0] == 9 || adealer.number[0] == 10{
                    if naiplayer.sum <= 12 {
                        println("AI give card")
                        randomcard = Int(rand()%numofcards)
                        randomcardnumber = randomcard%52
...
//        
//        while naiplayer.sum < 17 && naiplayer.sum < maxsum{
//            println("AI give card")
//            randomcard = Int(rand()%numofcards)
//            randomcardnumber = randomcard%52
//            while used[randomcard] == 0{
...
//        }
        for i in 0..<5{
        Pcards[i].hidden = true
        }
        for i in 0..<naiplayer.numberofcards {
            println(naiplayer.number[i])
            Pcards[i].image = GetImage(naiplayer.suit[i], n: naiplayer.number[i])
            Pcards[i].hidden = false
        
        }
        naiplayer.sum = countSum(naiplayer.number, size: naiplayer.numberofcards)
        if naiplayer.sum > 21 {
            Playernum.text = "AI player,Bust"
        } else{
            Playernum.text = "AI player"
        }
    ...
    func countdealer() {
        ...
        var maxsum:Int = 0
        if numofplayers-1 == 0 && aidied == 0{
            countaiplayer()
            maxsum = max(naiplayer.sum, players[0].sum)
        } else if aidied == 1{
            maxsum = players[currentplayer - 1].sum
    ...
    func winningcondition() {
        ...
        if adealer.sum > 21 {
            ...
            if numofplayers == 1 && aidied == 0{
                if naiplayer.sum <= 21{
                    naiplayer.money = naiplayer.money + 2 * 10
                    resultofgame += "AI player: \(naiplayer.sum) You Win! \n"
                
                } else {
                    resultofgame += "AI player: \(naiplayer.sum) You Lose! \n"
                    if naiplayer.money <= 0 {
                        println("AI Player Have No Money, GoodBye!")
                        resultofgame += "AI Player Have No Money, GoodBye! \n"
                        aidied = 1
                    }
            ...
            if numofplayers == 1 && aidied == 0{
                if naiplayer.sum < adealer.sum {
                    resultofgame += ("AI Player: \(naiplayer.sum) You Lose! \n")
                    if naiplayer.money <= 0 {
                        println("Your Have No Money, GoodBye!")
                        resultofgame += "AI Player You Have No Money, GoodBye! \n"
                        aidied = 1
                    }
                } else if naiplayer.sum == adealer.sum {
                    println("AI Player: \(naiplayer.number), Draw! \n")
                    resultofgame += ("AI Player: \(naiplayer).sum) Draw! \n")
                } else {
                    if naiplayer.sum > 21 {
                        println("AI Player: \(naiplayer.number) ,You Lose!")
                        resultofgame += ("AI Player: \(naiplayer.sum) You Lose! \n")
                        if naiplayer.money <= 0 {
                            println("Your Have No Money, GoodBye!")
                            resultofgame += "AI Player You Have No Money, GoodBye! \n"
                        ...
                        naiplayer.money = naiplayer.money + 2 * 10
                        println("AI Player: \(naiplayer.number) ,You Win!")
                        resultofgame += ("AI Player: \(naiplayer.sum) You Win!\n")
                    }
